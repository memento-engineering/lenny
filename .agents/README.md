...
- **Claude Code** reads `.claude/agents/` and `.claude/skills/`, which are
  ...
  `scripts/link-shared-agents.sh` (idempotent). So there is one copy, edited
  here.
...
- **`.claude/`** contains machine-local symlinks recreated by
  `scripts/link-shared-agents.sh`.

## Editing

Edit files **here**. The `.claude/` symlinks pick up changes automatically.
Add a new shared agent as `agents/<name>.agent.md` and symlink it into
