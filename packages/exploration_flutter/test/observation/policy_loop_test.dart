import 'package:exploration_flutter/src/contract/types.dart';
import 'package:exploration_flutter/src/observation/observation_request.dart';
import 'package:exploration_flutter/src/observation/policy_loop.dart';
import 'package:exploration_flutter/src/observation/stability_metadata.dart';
import 'package:exploration_flutter/src/stability/framework_busy_snapshot.dart';
...
List<MapEntry<String, BusyState>> _busyPlugins(
...
void main() {
  group('action-relative termination conditions', () {
    test('idle: framework + all plugins idle on first poll', () async {
      ...
      final _Script s = _Script(
        ...
        busy: <List<MapEntry<String, BusyState>>>[
          _busyPlugins(<({String ns, bool busy, String? reason, int? estMs})>[
      ...
      expect(tick.pluginsBusy, isEmpty);
    });
    ...
    test('budget: never idle, hits 800ms cap', () async {
      ...
      expect(tick.pluginsBusy, hasLength(1));
      expect(tick.pluginsBusy.first.namespace, 'p1');
      expect(tick.pluginsBusy.first.reason, 'loading');
      expect(tick.pluginsBusy.first.estMs, 200);
    });
  });
  ...
  group('quiet-frame N=2', () {
    ...
    test('busy frame breaks the streak; counter resets', () async {
      ...
      final _Script s = _Script(
        ...
        busy: <List<MapEntry<String, BusyState>>>[
          _busyPlugins(const <({String ns, bool busy, String? reason, int? estMs})>[]),
          _busyPlugins(const <({String ns, bool busy, String? reason, int? estMs})>[]),
          _busyPlugins(const <({String ns, bool busy, String? reason, int? estMs})>[]),
          _busyPlugins(const <({String ns, bool busy, String? reason, int? estMs})>[]),
          _busyPlugins(const <({String ns, bool busy, String? reason, int? estMs})>[]),
  ...
  group('bounded-stability', () {
    test('tags `budget` and reports plugins_busy on timeout', () async {
      ...
      expect(tick.pluginsBusy.single.namespace, 'q');
      expect(tick.pluginsBusy.single.reason, 'animating');
      expect(tick.pluginsBusy.single.estMs, 500);
    });
