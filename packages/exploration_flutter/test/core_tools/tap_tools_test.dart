...
void main() {
  testWidgets('core.tap dispatches SemanticsAction.tap on a button',
    ...
    final int id = recs
        .firstWhere((Map<String, Object> r) => r['label'] == 'Submit')['id']!
        as int;
  ...
  testWidgets('core.long_press dispatches SemanticsAction.longPress',
    ...
    final int id = cap.capture().firstWhere(
        (Map<String, Object> r) => r['label'] == 'Hold')['id']! as int;
    final CorePlugin plugin = CorePlugin(semantics: cap);
    final ExplorationTool lp = plugin.tools
        .firstWhere((ExplorationTool t) => t.name == 'long_press');
  ...
  test('schema_violation when node_id missing or wrong type', () async {
    ...
    final ToolResult wrong =
        await tap.call(<String, Object?>{'node_id': 'oops'});
  ...
  test('target_not_found on unknown id', () async {
    ...
    final ExplorationTool tap =
        plugin.tools.firstWhere((ExplorationTool t) => t.name == 'tap');
    final ToolResult r =
        await tap.call(<String, Object?>{'node_id': 9999});
    expect(r.ok, isFalse);
