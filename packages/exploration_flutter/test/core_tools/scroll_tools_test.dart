...
void main() {
  testWidgets('core.scroll axis+delta_pixels validation',
    ...
    final ExplorationTool scroll =
        plugin.tools.firstWhere((ExplorationTool x) => x.name == 'scroll');
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      final SemanticsCapture cap = SemanticsCapture();
      final CorePlugin plugin = CorePlugin(semantics: cap);
      final ExplorationTool sv = plugin.tools.firstWhere(
        (ExplorationTool x) => x.name == 'scroll_until_visible',
      );
  ...
  test('scroll_until_visible rejects max_iterations out of range', () async {
    final SemanticsCapture cap = SemanticsCapture();
    final CorePlugin plugin = CorePlugin(semantics: cap);
    final ExplorationTool sv = plugin.tools
        .firstWhere((ExplorationTool x) => x.name == 'scroll_until_visible');
    final ToolResult r = await sv.call(<String, Object?>{
      'scrollable_id': 1,
      'target_id': 2,
      'axis': 'vertical',
