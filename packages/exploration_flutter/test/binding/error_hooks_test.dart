...
class _RecordingPlugin extends ExplorationPlugin {
  _RecordingPlugin(this.namespace, {required this.handler});
...
void main() {
  ...
  final _RecordingPlugin firstPlugin =
      _RecordingPlugin('alpha', handler: (FlutterErrorDetails d) {
    firstCalls.add(d.exceptionAsString());
    return false;
  });

  // Second plugin: throws on every call.
  final _RecordingPlugin throwerPlugin =
      _RecordingPlugin('beta', handler: (FlutterErrorDetails _) {
    throw StateError('handler boom');
  });

  // Third plugin: claims errors whose message contains 'CLAIM'.
  final List<String> thirdCalls = <String>[];
  final _RecordingPlugin claimerPlugin =
      _RecordingPlugin('gamma', handler: (FlutterErrorDetails d) {
  ...
  setUpAll(() async {
    binding = ExplorationBinding.ensureInitialized(plugins: <ExplorationPlugin>[
      firstPlugin,
      throwerPlugin,
      claimerPlugin,
    ])!;
  ...
  test('error hooks are installed exactly once even after a second '
    ...
    final ExplorationBinding? again =
  ...
  test('every error is recorded into the ring regardless of handler return',
      () {
  ...
  test('FlutterError.onError wrapper records, dispatches, and forwards', () {
    ...
    expect(firstCalls, hasLength(1),
        reason: 'first plugin handler must run');
    expect(thirdCalls, hasLength(1),
        reason: 'thrower plugin handler exception must not block subsequent '
            'plugin handlers');
  });
  ...
  test('PlatformDispatcher.onError wrapper records and forwards', () {
    ...
    expect(handled, isTrue,
        reason: 'when no prior platform onError exists the wrapper '
            'reports handled (matches framework default)');
  });

  test('ring entry shape matches schema', () {
    final int beforeSeq = binding.debugHighestErrorSeq();
    binding.debugAppendError('shape-check', StackTrace.fromString(
        '#0 a\n#1 b\n#2 c\n#3 d\n#4 e\n#5 f\n#6 g'));
    ...
    expect(e.frames.length, 5,
        reason: 'frames are capped at the first 5');
    expect(e.frames.first, '#0 a');
    expect(e.wallClockOffsetMs, isA<int>());
    expect(e.wallClockOffsetMs, greaterThanOrEqualTo(0));
  });

  test('pluginRegistry getter exposes the wired registry', () {
