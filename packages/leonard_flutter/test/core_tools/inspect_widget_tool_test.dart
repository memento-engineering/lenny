...
void main() {
  testWidgets('core.inspect_widget returns a depth-capped semantics subtree',
    ...
    final int id = recs
        .firstWhere((Map<String, Object> r) => r['label'] == 'A')['id']! as int;
    final CoreExtension plugin = CoreExtension(semantics: cap);
    final LeonardTool t = plugin.tools
        .firstWhere((LeonardTool x) => x.name == 'inspect_widget');
    final ToolResult r =
        await t.call(<String, Object?>{'node_id': id, 'depth': 3});
  ...
  test('schema_violation on bad depth', () async {
    ...
    final LeonardTool t = plugin.tools
        .firstWhere((LeonardTool x) => x.name == 'inspect_widget');
    final ToolResult r =
        await t.call(<String, Object?>{'node_id': 1, 'depth': 20});
    expect(r.ok, isFalse);
  ...
  test('target_not_found on unknown id', () async {
    ...
    final LeonardTool t = plugin.tools
        .firstWhere((LeonardTool x) => x.name == 'inspect_widget');
    final ToolResult r =
        await t.call(<String, Object?>{'node_id': 9999});
    expect(r.ok, isFalse);
