...
void main() {
  ...
  test('release-mode gating: extension is debug/profile-only', () {
    ...
    // well-defined.
    expect(
      _diagnosticsExt.startsWith('$kLeonardExtensionPrefix.core.'),
      isTrue,
    );
    if (kReleaseMode) {
  ...
  test('double call returns cached results without re-walking', () async {
    ...
    final String first = await binding.invokeServiceExtension(
      _diagnosticsExt,
      const {},
    );
    ...
    final String second = await binding.invokeServiceExtension(
      _diagnosticsExt,
      const {},
    );
    expect(
  ...
  test(
    'throwing root yields ok:false with empty results, no rethrow',
    () async {
      binding.debugSetDiagnosticsRootProviderForTesting(() {
