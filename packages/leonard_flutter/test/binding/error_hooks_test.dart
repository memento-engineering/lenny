...
void main() {
  ...
  final _RecordingExtension firstPlugin =
      _RecordingExtension('alpha', handler: (FlutterErrorDetails d) {
    firstCalls.add(d.exceptionAsString());
    return false;
  });

  // Second plugin: throws on every call.
  final _RecordingExtension throwerPlugin =
      _RecordingExtension('beta', handler: (FlutterErrorDetails _) {
    throw StateError('handler boom');
  });

  // Third plugin: claims errors whose message contains 'CLAIM'.
  final List<String> thirdCalls = <String>[];
  final _RecordingExtension claimerPlugin =
      _RecordingExtension('gamma', handler: (FlutterErrorDetails d) {
