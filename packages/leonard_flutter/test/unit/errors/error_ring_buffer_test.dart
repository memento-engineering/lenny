...
void main() {
  group('ErrorRingBuffer', () {
    ...
    test('asserts capacity > 0', () {
      expect(
        () => ErrorRingBuffer(capacity: 0, sessionClock: sw),
        throwsA(isA<AssertionError>()),
      );
    });

    test('adds entries with monotonic seq starting at 1', () {
      final ErrorRingBuffer buf = ErrorRingBuffer(
        capacity: 10,
        sessionClock: sw,
      );
    ...
    test('evicts oldest entry when capacity is exceeded', () {
      ...
      expect(buf.entries.map((ErrorEntry e) => e.message).toList(), <String>[
        'b',
        'c',
        'd',
      ]);
      expect(buf.entries.map((ErrorEntry e) => e.seq).toList(), <int>[2, 3, 4]);
      expect(
        buf.highestSeq,
        4,
    ...
    test('entriesSince returns suffix strictly newer than cursor', () {
      ...
      buf.add('b', null);
      buf.add('c', null);
      expect(buf.entriesSince(0).map((ErrorEntry e) => e.seq).toList(), <int>[
        1,
        2,
        3,
      ]);
      expect(buf.entriesSince(2).map((ErrorEntry e) => e.seq).toList(), <int>[
        3,
      ]);
    ...
    test('wallClockOffsetMs uses provided session clock', () async {
      ...
      expect(
        second.wallClockOffsetMs,
        greaterThanOrEqualTo(first.wallClockOffsetMs),
      );
    });
  });
}
