...
class _Healthy extends LeonardExtension with PerceptionExtension {
  const _Healthy();
  @override
...
class _BigFragment extends LeonardExtension with PerceptionExtension {
  const _BigFragment();
  @override
...
void main() {
  ...
  group('DiagnosticsHotPathRegression', () {
    test('get_stable_observation carries no diagnostics_tree key and stays '
        'byte-identical across a get_diagnostics_tree call', () async {
      if (!kDebugMode) return;
      await Future<void>.delayed(Duration.zero);
      ...
      for (final Object? fragment
        ...
        expect(
          (fragment! as Map<String, Object?>).containsKey('diagnostics_tree'),
          isFalse,
        );
      }
