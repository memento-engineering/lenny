...
void main() {
  ...
  Future<ProcessResult> install(List<String> args) => Process.run('dart', <
    String
  >[
    'bin/install.dart',
    '--dir',
  ...
  test('bare install lands the canonical .agents/ copy only', () async {
    ...
    expect(
      File('${tmp.path}/.agents/skills/drive-with-leonard/SKILL.md')
          .existsSync(),
      isTrue,
    );
    expect(
  ...
  test('--copilot overlays .github/agents/ and nothing at the repo root', () async {
    final ProcessResult r = await install(const <String>['--copilot']);
    expect(r.exitCode, 0, reason: r.stderr.toString());
    final String linkPath = '${tmp.path}/.github/agents/leonard-drive.agent.md';
    expect(
  ...
  test('--claude overlays .claude/{agents,skills} as resolving links', () async {
    ...
    expect(
      File('${tmp.path}/.claude/skills/drive-with-leonard/SKILL.md')
          .existsSync(),
      isTrue,
    );
  });

  test('install is idempotent — a second run skips, never duplicates', () async {
