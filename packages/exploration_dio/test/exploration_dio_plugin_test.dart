...
(ExplorationDioPlugin, Dio, _HangingAdapter) _make() {
  final adapter = _HangingAdapter();
  final dio = Dio()..httpClientAdapter = adapter;
  final plugin = ExplorationDioPlugin(dio);
  return (plugin, dio, adapter);
}

Future<void> _init(ExplorationDioPlugin p) async {
  await p.initialize(PluginContext(
    namespace: 'dio',
