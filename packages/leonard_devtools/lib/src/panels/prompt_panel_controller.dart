...
/// Builds an [LeonardSession]. The closure owns whatever connection
/// it needs — production wires a closure over `serviceManager.service` +
/// the main isolate id (via [LeonardSession.fromVmService]); tests
/// inject a fake.
typedef SessionFactory = Future<LeonardSession> Function();

/// Drives the in-panel session lifecycle for [PromptPanel].
///
/// Owns:
///   - the [LeonardSession] handshake / lifecycle,
...
class PromptPanelController {
  ...
  /// the perception-action loop via [LeonardSession.run].
  ///
  ...
  Future<void> start(
    ...
    await session.start(panelCfg.goal, panelCfg.toLeonardConfig());
