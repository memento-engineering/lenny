...
void main() {
  group('TurnDetailView', () {
    testWidgets('renders action, validation, reasoning and route stack', (
      tester,
    ) async {
    ...
    testWidgets('reports rejected validation with reason and retries', (
      tester,
    ) async {
    ...
    testWidgets('falls back to "(none)" when no reasoning provided', (
      tester,
    ) async {
      await tester.pumpWidget(
        _wrap(TurnDetailView(record: _turn(modelMetadata: const {}))),
      );
    ...
    testWidgets(
      'semantics JSON is not rendered until expansion tile is tapped',
      (tester) async {
        await tester.pumpWidget(_wrap(TurnDetailView(record: _turn())));
    ...
    testWidgets('screenshot decoded only when expanded', (tester) async {
      ...
      await tester.pumpWidget(
        _wrap(TurnDetailView(record: record, screenshotDecoder: recorder)),
      );
