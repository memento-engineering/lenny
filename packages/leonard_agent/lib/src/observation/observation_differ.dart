...
class ObservationDiffer {
  ...
  static Map<String, ExtensionDiff> _pluginsDiff(
    ...
    final Map<String, ExtensionDiff> out = <String, ExtensionDiff>{};
    for (final String ns in c.keys) {
      final ExtensionFragment curr = c[ns]!;
      final ExtensionFragment? prev = p[ns];
      if (prev == null) {
        out[ns] = ExtensionDiffAdded(current: curr.data);
        continue;
      }
      if (curr.deltaFriendly && prev.deltaFriendly) {
        out[ns] = _structuredDiff(prev.data, curr.data);
      } else {
        out[ns] = ExtensionDiffOpaque(previous: prev.data, current: curr.data);
      }
    }
    for (final String ns in p.keys) {
      if (!c.containsKey(ns)) {
        out[ns] = ExtensionDiffRemoved(previous: p[ns]!.data);
      }
    }
    return out;
  }

  static ExtensionDiffStructured _structuredDiff(
