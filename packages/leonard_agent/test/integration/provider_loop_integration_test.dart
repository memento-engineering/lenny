...
/// `packages/leonard_agent/test/provider/swift_infer/swift_infer_provider_test.dart`
...
class _Scenario {
  ...
  static final _Scenario happyPath = _Scenario(
    ...
    regressionNote:
        ...
        'and routes through invokeExtensionTool.',
  );
...
void main() {
  ...
  for (final _Scenario scenario in _Scenario.values) {
    test('provider loop scenario: ${scenario.id}', () async {
      ...
      // LeonardVmServiceFake is a VmService; fromVmService accepts VmService.
      final LeonardSession session = LeonardSession.fromVmService(
        fake,
        'isolate-0',
      );
      await session.start('exploration goal', const LeonardConfig());
