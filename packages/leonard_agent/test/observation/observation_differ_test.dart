...
ExtensionFragment _frag(String ns, Map<String, dynamic> data,
...
void main() {
  ...
  test('delta-friendly plugin yields ExtensionDiffStructured', () {
    final Observation prev = _obs(plugins: <String, ExtensionFragment>{
      'router': _frag('router', <String, dynamic>{'path': '/a'},
          deltaFriendly: true),
    });
    final Observation curr = _obs(plugins: <String, ExtensionFragment>{
      'router': _frag('router', <String, dynamic>{'path': '/b'},
          deltaFriendly: true),
    });
    final ObservationDiff d = ObservationDiffer.diff(prev, curr);
    expect(d.plugins['router'], isA<ExtensionDiffStructured>());
    final ExtensionDiffStructured s = d.plugins['router']! as ExtensionDiffStructured;
  ...
  test('plugin namespace insertion order does not affect output JSON', () {
    final Observation a = _obs(plugins: <String, ExtensionFragment>{
      'zeta': _frag('zeta', <String, dynamic>{'a': 1}),
      'alpha': _frag('alpha', <String, dynamic>{'a': 1}),
    });
    final Observation b = _obs(plugins: <String, ExtensionFragment>{
