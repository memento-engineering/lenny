...
Observation _obs({
  ...
  Map<String, ExtensionFragment> extensions =
      const <String, ExtensionFragment>{},
}) {
  return Observation(
    core: CoreFragment(
...
void main() {
  ...
  test('delta-friendly extension yields ExtensionDiffStructured', () {
    final Observation prev = _obs(
      extensions: <String, ExtensionFragment>{
        'router': _frag('router', <String, dynamic>{
          'path': '/a',
    ...
    final ObservationDiff d = ObservationDiffer.diff(prev, curr);
    expect(d.extensions['router'], isA<ExtensionDiffStructured>());
    final ExtensionDiffStructured s =
        d.extensions['router']! as ExtensionDiffStructured;
    expect(s.changed.keys, equals(<String>{'path'}));
    expect(s.changed['path']!.curr, equals('/b'));
    expect(s.changed['path']!.prev, equals('/a'));
  });

  test('opaque extension yields previous+current pair', () {
    final Observation prev = _obs(
      extensions: <String, ExtensionFragment>{
  ...
  test(
    ...
    () {
      ...
      final ObservationDiff d = ObservationDiffer.diff(prev, curr);
      expect(d.extensions['p'], isA<ExtensionDiffOpaque>());
    },
  );
  ...
  test('namespace in curr only is ExtensionDiffAdded', () {
    ...
    final ObservationDiff d = ObservationDiffer.diff(prev, curr);
    expect(d.extensions['x'], isA<ExtensionDiffAdded>());
  });
  ...
  test('namespace dropped from curr is ExtensionDiffRemoved', () {
    ...
    final Observation curr = _obs();
    final ObservationDiff d = ObservationDiffer.diff(prev, curr);
    expect(d.extensions['x'], isA<ExtensionDiffRemoved>());
  });
  ...
  test('extension namespace insertion order does not affect output JSON', () {
