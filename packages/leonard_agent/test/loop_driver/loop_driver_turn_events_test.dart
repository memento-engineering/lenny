...
void main() {
  test('driver emits TurnThinking → TurnActionDecided → TurnValidation → '
      'TurnComplete in order', () async {
    ...
    final thinkingCount = events.whereType<TurnThinking>().length;
    ...
    final firstActionIdx = events.indexWhere((e) => e is TurnActionDecided);
    final lastThinkingIdx = events.lastIndexWhere((e) => e is TurnThinking);
    ...
    expect(
      tail.map((e) => e.runtimeType.toString()).toList(),
      equals(<String>[
        'TurnActionDecided',
        'TurnValidation',
        'TurnUsage',
  ...
  test('thinking subscription is cancelled when decide returns '
      '(no orphan listeners)', () async {
    final sink = _MemorySink();
    final writer = await _newWriter(sink);
    final host = _FakeHost(tools: <ToolDescriptor>[_coreWait()]);
    ...
    provider.pushThinking(const ThinkingDelta(text: 'a', isFinal: false));
    await driver.runTurn();
    await Future<void>.delayed(Duration.zero);
    ...
    provider.pushThinking(const ThinkingDelta(text: 'b', isFinal: false));
    await Future<void>.delayed(Duration.zero);
