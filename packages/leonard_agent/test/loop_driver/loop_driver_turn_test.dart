...
class _FakeHost implements LoopHost {
  ...
  /// Tools list (constant unless mutated by disableExtension).
  ...
  void disableExtension(String namespace, String reason) {
    calls.add('disableExtension:$namespace');
  ...
  Set<String> activeExtensionNamespaces() {
    calls.add('activeExtensionNamespaces');
...
void main() {
  group('LoopDriver.runTurn', () {
    test('happy path: 10 steps execute in order', () async {
      ...
      // Observable call sequence: observe → activeExtensionNamespaces (during
      // strike accounting; called at least once) → mergedTools (prompt) →
      // decide → executeAction → notifyExtensions. The exact order is the
      // load-bearing part.
      final calls = host.calls;
      // Drop bookkeeping calls (activeExtensionNamespaces) for ordering check.
