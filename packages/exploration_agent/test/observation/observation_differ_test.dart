...
Observation _obs({
  ...
  Map<String, PluginFragment> plugins = const <String, PluginFragment>{},
}) {
  return Observation(
    core: CoreFragment(
...
PluginFragment _frag(String ns, Map<String, dynamic> data,
...
Observation _bigObs() => _obs(
      ...
      plugins: <String, PluginFragment>{
        'router': _frag('router', <String, dynamic>{'path': '/home'},
            deltaFriendly: true),
...
Observation _bigObs2() => _obs(
      ...
      plugins: <String, PluginFragment>{
        'router': _frag('router', <String, dynamic>{'path': '/details'},
            deltaFriendly: true),
...
void main() {
  test('first-turn diff treats empty prior as all-added', () {
    final Observation curr = _obs(nodes: <int, SemanticsNode>{
      1: _node(1),
      2: _node(2),
    });
    final ObservationDiff d = ObservationDiffer.diff(Observation.empty(), curr);
    expect(d.core.nodesAdded.map((SemanticsNode n) => n.id).toList(),
        equals(<int>[1, 2]));
  ...
  test('node added/removed/changed split correctly', () {
    final Observation prev = _obs(nodes: <int, SemanticsNode>{
      1: _node(1, label: 'a'),
      2: _node(2),
    });
    final Observation curr = _obs(nodes: <int, SemanticsNode>{
  ...
  test('delta-friendly plugin yields PluginDiffStructured', () {
    final Observation prev = _obs(plugins: <String, PluginFragment>{
      'router': _frag('router', <String, dynamic>{'path': '/a'},
          deltaFriendly: true),
    });
    final Observation curr = _obs(plugins: <String, PluginFragment>{
      'router': _frag('router', <String, dynamic>{'path': '/b'},
          deltaFriendly: true),
    });
    final ObservationDiff d = ObservationDiffer.diff(prev, curr);
    expect(d.plugins['router'], isA<PluginDiffStructured>());
    final PluginDiffStructured s = d.plugins['router']! as PluginDiffStructured;
  ...
  test('opaque plugin yields previous+current pair', () {
    ...
    final ObservationDiff d = ObservationDiffer.diff(prev, curr);
    expect(d.plugins['opaque'], isA<PluginDiffOpaque>());
    final PluginDiffOpaque o = d.plugins['opaque']! as PluginDiffOpaque;
  ...
  test(
    ...
    final ObservationDiff d = ObservationDiffer.diff(prev, curr);
    expect(d.plugins['p'], isA<PluginDiffOpaque>());
  });

  test('namespace in curr only is PluginDiffAdded', () {
    final Observation prev = _obs();
    final Observation curr = _obs(plugins: <String, PluginFragment>{
      'x': _frag('x', <String, dynamic>{'k': 1}),
    });
    final ObservationDiff d = ObservationDiffer.diff(prev, curr);
    expect(d.plugins['x'], isA<PluginDiffAdded>());
  });

  test('namespace dropped from curr is PluginDiffRemoved', () {
    final Observation prev = _obs(plugins: <String, PluginFragment>{
      'x': _frag('x', <String, dynamic>{'k': 1}),
    });
    final Observation curr = _obs();
    final ObservationDiff d = ObservationDiffer.diff(prev, curr);
    expect(d.plugins['x'], isA<PluginDiffRemoved>());
  });

  test('errorsAdded reports new seq numbers without re-reporting old', () {
    final Observation prev = _obs(errors: <RuntimeError>[
      const RuntimeError(
          seq: 1, message: 'old', frames: <String>[], wallClockOffsetMs: 0),
    ]);
    final Observation curr = _obs(errors: <RuntimeError>[
      const RuntimeError(
          seq: 1, message: 'old', frames: <String>[], wallClockOffsetMs: 0),
      const RuntimeError(
          seq: 2, message: 'new', frames: <String>[], wallClockOffsetMs: 5),
    ]);
    final ObservationDiff d = ObservationDiffer.diff(prev, curr);
  ...
  test('plugin namespace insertion order does not affect output JSON', () {
    final Observation a = _obs(plugins: <String, PluginFragment>{
      'zeta': _frag('zeta', <String, dynamic>{'a': 1}),
      'alpha': _frag('alpha', <String, dynamic>{'a': 1}),
    });
    final Observation b = _obs(plugins: <String, PluginFragment>{
    ...
    final String encAgain =
        jsonEncode(ObservationDiffer.diff(a, b).toJson());
