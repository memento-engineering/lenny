...
import 'package:exploration_agent/src/trajectory/reader.dart';
import 'package:exploration_agent/src/trajectory/records.dart';
import 'package:exploration_agent/src/trajectory/writer.dart';
import 'package:exploration_agent/src/trajectory/sink.dart';
...
void main() {
  group('TrajectoryReader.readAll', () {
    test('round-trips a 4-record fixture written by TrajectoryWriter', () async {
      final sink = _RecordingSink();
      final w = TrajectoryWriter(sink);
      await w.writeHeader(_hdr());
      await w.writeTurn(_turn(0));
      await w.writePluginDisabled(const PluginDisabledEvent(
        namespace: 'dio',
      ...
      final disabled = records[2] as PluginDisabledEvent;
    ...
    test('missing type discriminator yields UnknownTrajectoryRecord(rawType: "null")',
        () {
  ...
  group('TrajectoryReader.readStream', () {
    test('emits records lazily from a line stream', () async {
      ...
      final records =
          await TrajectoryReader.readStream(lines).toList();
