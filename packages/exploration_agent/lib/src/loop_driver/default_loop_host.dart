...
/// an [ExplorationSession] (and its underlying `VmServiceClient`) plus
/// caller-supplied tool descriptors.
///
/// The handshake's `PluginManifestEntry` list only carries
...
/// app's `ExplorationBinding` during the same `executeAction`
/// round-trip, so the harness-side [notifyPlugins] is intentionally a
...
class DefaultLoopHost implements LoopHost {
  ...
  DefaultLoopHost.fromSession({
    ...
    required Map<String, List<ToolDescriptor>> pluginTools,
  ...
  final ExplorationSession _session;
  ...
  List<ToolDescriptor> mergedTools() {
    ...
    for (final String ns in activePluginNamespaces()) {
  ...
  void disablePlugin(String namespace, String reason) {
    ...
    // Repeated disables are no-ops: PluginAutoDisabled fires exactly
  ...
  Future<void> notifyPlugins(
    ...
    // (`PluginRegistry.onActionExecutedAll`) runs in-process inside the
