...
class InMemoryPromptPanelConfigStore implements PromptPanelConfigStore {
  ...
  Future<void> save(
    ...
    _cell = <String, dynamic>{
      ...config.toJson(),
      'knownPluginNamespaces': knownNamespaces.toList(),
    };
  }
}
...
PromptPanelConfig _reconcile(
  ...
  final known = Set<String>.from(
    (json['knownPluginNamespaces'] as List<dynamic>?) ?? const <dynamic>[],
  );
  final enabled = cfg.enabledPluginNamespaces;
  ...
  return PromptPanelConfig(
    ...
    enabledPluginNamespaces: reconciled,
  );
}
