...
PromptPanelConfig _cfg({
  ...
  Set<String> enabled = const <String>{},
    ...
    PromptPanelConfig(
      ...
      enabledPluginNamespaces: enabled,
    );
...
void main() {
  group('InMemoryPromptPanelConfigStore', () {
    ...
    test('new namespace (not in known) defaults to enabled', () async {
      ...
      expect(loaded!.enabledPluginNamespaces, containsAll(['router', 'newPlugin']));
    });
    ...
    test('disabled namespace stays disabled', () async {
      ...
      expect(loaded!.enabledPluginNamespaces, contains('router'));
      expect(loaded.enabledPluginNamespaces, isNot(contains('dio')));
    });
    ...
    test('removed namespace dropped silently', () async {
      ...
      expect(loaded!.enabledPluginNamespaces, {'router'});
      expect(loaded.enabledPluginNamespaces, isNot(contains('old')));
    });
  });
