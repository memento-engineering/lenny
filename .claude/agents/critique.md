...
- The path to the rubric reference file: `skills/critique/references/<rubric>.md`.
- The path to the output schema: `skills/critique/references/output-schema.json`.

## Flow

1. **Read the bead.** `bd show <id>` — capture the description, design, acceptance criteria, implementation plan, and validation plan.
2. **Read the rubric.** Read `skills/critique/references/<rubric>.md`. Do **not** read any other file under `skills/critique/references/` except `output-schema.json` — peer rubric texts would anchor your grade on cross-rubric context you are not supposed to have.
...
The isolation rule from step 2 still applies: do **not** read any other `skills/critique/references/*.md` file. Re-deliberation evidence is the branch + the verdict comment, not a peer rubric's text.
5. **Emit.** Print exactly one JSON object on stdout, no prose around it, conforming to `skills/critique/references/output-schema.json`:
...
- **Required:** `skills/critique/references/<rubric>.md` (the named rubric only).
...
- **Forbidden:** any other file under `skills/critique/references/` (other rubric texts), except `skills/critique/references/output-schema.json`. Reading peer rubric texts is what "in isolation" forbids — not reading the codebase or backlog.
