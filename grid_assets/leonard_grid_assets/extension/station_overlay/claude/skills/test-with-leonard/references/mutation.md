...
- `leonard_native` on butcher, the cut-over run of 2026-09-24: 663 mutants — 457 killed, 101 survived, 96 uncovered, 7 that did not compile and 2 that timed out — for a 69.88% mutation score and 81.90% over covered code, in 226 seconds of dry sizing plus the scored phase. Survivors concentrate: 31 in `xcuitest_backend.dart`, 24 in `mac2_backend.dart`, 23 in `uiautomator2_backend.dart`, 14 in `native_extension.dart`, 9 across the other seven files. The earlier numbers are not comparable, because the engine that produced them scored every non-compiling mutant as a kill and never separated an uncovered mutant from a survivor: a 2026-09-09 run reported 502 mutants, 114 survivors and 77.29% in 1h50m19s, and a 2026-08-01 run reported 542 mutants, 215 survivors and 60.33%. Survivor triage on those runs produced the initial four checks and found `lenny-i2j6`.
- `leonard_contract`, measured on the retired regex engine: 55 mutants, 28 survivors, 49.09% killed, rating D, 33s. Tests authored from the recorded checks reached 54/55 killed, 98.18%, rating B. The remaining mutant exposed the incidental-equivalence design finding.
- `leonard_flutter`, which still runs the regex engine: a targeted, non-random sample of 126 of 777 mutants across five files killed 77.78%, rating C, in 63m. Three of five predictions were wrong: the shipped fake killed 13/13, `budgeted_json.dart` killed 8/8, and `error_ring_buffer.dart` killed 5/5. That contradiction weakened categories from predictions into checks and added the tolerant-consumer rule.

Mutation rewrites source in place. Never analyze, stage, or commit from a tree with a live run. Read the score honestly; decouple informational CI outcome from score thresholds rather than configuring reports to claim success. Use the installed `tool/leonard/run_mutation.sh` for pure-Dart packages: it drives butcher, an AST engine, and writes a Stryker JSON report. Flutter mutation remains package-scoped, on the older regex engine and its own local path, because butcher has no `flutter test` runner seam yet and the observed per-mutant cost there was roughly 60 times higher.
