...
**Changes from v0.3:**
- Replaced `go_router` with raw `Router`/`Navigator` as the routing reference extension. Simpler, exercises the contract more honestly, no opinionated framework dependency.
...
- Aligned extension discovery convention with the emerging Dart `extension/` pattern from Jake MacDonald's "Packaged AI Assets" proposal (extension/exploration/config.yaml). Implementation still uses explicit registration in v1, but the convention is in place for future auto-discovery.
- Added a new §8 on resources and prompts as a deferred addition to the extension contract, mirroring the MCP Resources/Prompts shape from the Packaged AI Assets design.
- Clarified that extension updates are not breaking changes (consistent with the Packaged AI Assets posture).
- Removed the deprecation-warnings open question.

**Changes from v0.2:** Restructured around a host-and-extensions architecture. The host binding is small and policy-free; extensions contribute tools, observations, and lifecycle hooks via a typed Dart contract built on the VM service extension protocol. This reframes the Navigator 2.0 problem as a routing-extension concern rather than a core-binding problem, and creates a path for third-party packages (state management, networking, custom design systems, even Marionette itself) to integrate without forking the project.

---

## 1. Summary

A Dart-based agent harness that drives a running Flutter application through a custom `WidgetsBinding`. The binding is small and opinionated about almost nothing: it registers an extension contract on top of the VM service extension protocol, coordinates the perception-action loop, and gets out of the way. Tools, observation fragments, and lifecycle hooks come from extensions.

The harness operates against the *real running app*, not a test entrypoint. Real network, real time, real animations. Stability is defined by the harness using observable frame-lifecycle signals plus extension-contributed busy-state, not by `pumpAndSettle`.

The deliverable is the perception-action primitive plus the extension contract. The interesting capabilities — routing-aware navigation, state-graph observation, network-aware stability — come from extensions, written by us for v1 and by the ecosystem over time.
...
- Ship with reference extensions that demonstrate the contract and solve concrete problems (routing, state, networking).

### Non-goals

- Generating new Flutter code, including new tests. The agent only acts on the running app.
- Replacing the Dart MCP server or Marionette MCP. Those are for IDE-side workflows; this is for harness work where the user controls the loop end to end. (An extension that exposes Marionette-equivalent functionality is in scope; replacing Marionette is not.)
...
- Solving every routing/state/networking framework. We solve the contract; extensions solve the framework-specific bits.
...
- **Extension authors** — package maintainers (routing libraries, state management, design systems) who want their package to be visible to and controllable by the exploration agent.

The first audience shapes the host. The extension authors shape the ecosystem.
...
**Extension.** A Dart package that implements the extension contract (§7). Contributes some combination of tools, observation fragments, and lifecycle hooks. Lives in the user's app.

**Turn.** One iteration of the perception-action loop.

**Stability policy.** The host's rule for when to capture an observation. Considers framework signals (transient callbacks, microtasks) and extension-contributed busy-state.

**Observation.** The host's snapshot of the app at a turn boundary. A merged structure: core fragment (semantics tree, route stack, errors, stability metadata) plus extension-contributed fragments.

**Action.** A typed call to a tool. Some tools come from the host (semantics-tree-driven tap, enter_text, scroll, screenshot); most come from extensions.

**Diff.** Mechanical, host-authored description of what changed between observations. Per-turn, includes extension-contributed fragments.
...
- Hosts an extension registry. Extensions register with the binding at initialization time.
- Exposes a `getStableObservation(policy)` extension that merges the core fragment with extension-contributed fragments.
- Hooks into `SchedulerBinding` to track transient callbacks, microtasks, and frame commits.
- Maintains a bounded ring buffer of recent runtime errors via `FlutterError.onError` and `PlatformDispatcher.instance.onError`. Extensions that want their own error handling chain through the host's hook (§7.5).
...
- Screenshots
- The extension contract enforcement (namespacing, budgets, lifecycle ordering)
...
- Anything about specific custom design systems beyond the configuration knobs Marionette also exposes (which can themselves be an extension in this design)
...
**Prompt panel.** A text field for the goal, a "Start" button, and configuration controls (model selection, session budgets, extension enablement). Replaces the CLI's argument parsing for interactive use. The developer types what they want the agent to do and hits Start.
...
**Timeline panel.** A scrollable list of turns. Each row shows the action taken, a one-line summary of the diff, and the running summary at that point. Click a turn to expand it into a detail view: the full observation snapshot (semantics tree, route stack, extension fragments, screenshot if enabled), the model's reasoning, the action and validation result. The timeline updates live as the session progresses and is also browsable after the session ends.

The panel deliberately does not duplicate everything the CLI does. It does not yet expose: long-form trajectory analytics, batch session runs, headless replay (v2 anyway), or extension authoring tools. Those remain CLI-only or out of v1 entirely.
...
**It distributes ecosystem work.** We can't ship and maintain support for every library. With an extension contract, package maintainers can ship integration extensions. We ship the host, the contract, and reference extensions that target Flutter's built-in primitives.
...
abstract class LeonardExtension {
  ...
  /// Tools this extension contributes.
  List<LeonardTool> get tools;

  /// Called once at host initialization, before any observation or action.
  /// Extensions register VM service extensions, install hooks, etc., here.
  ...
  /// `observation.extensions[namespace]`. Returns null to contribute nothing.
  Future<Map<String, Object?>?> observe(ObservationContext context);

  /// Called by the stability policy to ask whether the extension considers
  /// the app busy. Used to delay observation capture until extension-known
  /// async work completes.
  Future<BusyState> busyState();

  /// Called after each action is executed. Extensions use this to record
...
An extension's tools are typed Dart functions exposed to the agent:
...
Tool names are mandatorily namespaced — the host enforces `<extension_namespace>.<tool_name>` at registration. Collisions are rejected at startup.

The host merges tools from all extensions with its own core tools (`tap`, `enter_text`, etc., which live in the reserved `core` namespace) into a single tool list presented to the model.

### 7.3 Observation fragments

Extensions return structured Dart maps from `observe()`. The host merges these into the observation:
...
The host enforces a per-extension observation budget (default 1KB serialized, configurable). Extensions that exceed the budget have their fragment truncated with a warning. Extensions should return `null` when they have nothing relevant to contribute (for example, a routing extension should return null when not on a route it manages).

### 7.4 Stability contributions

Extensions return a `BusyState` from `busyState()`:
...
The host's stability policies (§9) treat any `isBusy: true` from any extension as a reason to keep waiting, up to the policy's wall-clock budget. If the budget expires with extensions still busy, the observation is captured anyway and tagged with the busy extensions and their reasons.
...
Extensions that need framework-level hooks can request them through `ExtensionContext` rather than subclassing the binding:
...
This is the mechanism that lets extensions do framework-level things without subclassing the binding (which would conflict with the host's binding ownership).

### 7.6 Extension registration

Extensions are registered at host initialization:
...
Extensions are user-instantiated, which is the only realistic option in Dart's compilation model. We don't try to auto-discover extensions from `pubspec.yaml` in v1; we expect users to wire them up explicitly. The CLI can scaffold this for common configurations.

**Discovery convention.** Extension packages should declare their presence via `extension/exploration/config.yaml`, following the same pattern as Jake MacDonald's "Packaged AI Assets" proposal for the Dart MCP server. This is purely conventional in v1 — the host doesn't read it — but adopting the convention now means that if we later add auto-discovery, well-behaved extension packages are already discoverable. The config declares the extension's namespace, the Dart class to instantiate, and any constructor requirements the user must satisfy. A v2 auto-discovery mechanism could read these manifests, generate an extension-instantiation file, and reduce the explicit-registration burden in `main()`.
...
Extension updates that add tools, expand observation fragments, or refine busy-state heuristics are **not breaking changes**. Extension authors should release as often as they want. The host treats unfamiliar fields in observation fragments as opaque and passes them through; new tools simply appear in the merged tool list. Breaking changes are reserved for changes to the extension contract itself (the `LeonardExtension` interface), which the host versions explicitly.
...
### 7.8 Extension ordering and isolation

Extensions run in registration order for `observe()`, `busyState()`, `onActionExecuted()`, and `dispose()`. The host catches and logs exceptions from any extension method without aborting the session — a misbehaving extension degrades to "no contribution this turn" rather than killing the whole loop.

An extension's failure does not affect another extension's contribution. Each `observe()` call runs in its own try/catch. An extension that throws three times in a row is automatically disabled for the rest of the session, with a clear log entry.

## 8. Resources and prompts (deferred to v2)

The extension contract above covers tools (agent-invoked), observations (host-merged context), and lifecycle hooks. Two adjacent capabilities exist in the MCP world that we deliberately defer:

**Resources.** Static or dynamic content an extension makes available to the agent on demand or by reference (e.g., `@routes` to inject a list of all registered routes into the prompt). These are passive — the agent reads them; they don't drive action.
...
- An extension author needs to inject app-specific reference material into the agent's context that doesn't fit cleanly into the per-turn observation budget. (Today they can just put it in their observation fragment; the budget enforcement is the only real reason to want a separate resource channel.)

**The extension contract should leave room for these.** Specifically, the `extension/exploration/config.yaml` manifest should use a structure compatible with adding `resources:` and `prompts:` keys later, and the extension Dart interface should be able to add `List<LeonardResource>` and `List<LeonardPrompt>` getters in a backward-compatible way. Extensions that author resources/prompts for a coding-time agent today (via Packaged AI Assets) should be able to share the same source files when our resource/prompt support arrives.
...
This replaces `pumpAndSettle` for the same reasons as v0.2 (real apps never settle in the test-framework sense) and adds extension-contributed busy-state.

### 9.1 Stability policies

Three default policies, each composing framework signals with extension contributions:

**Action-relative (default).** Wait until any of: route stack changes, semantics tree changes, *no extension reports busy*, or 800ms wall-clock. Whichever first. Best matches "did my action do something visible?" and terminates promptly.

**Quiet-frame.** Wait until N consecutive frames have committed with no transient callbacks, no pending microtasks, *and no extension reports busy* (default N=2). Terminates only when the app is genuinely idle. Useful for first observation of a session.
...
1. **Stabilize.** Host calls `getStableObservation(policy)`, which polls framework signals and `busyState()` from all extensions until the policy terminates.
2. **Capture core fragment.** Semantics tree, route stack (best-effort Navigator 1.0), errors, stability metadata.
3. **Capture extension fragments.** Host calls `observe()` on each extension, applies budget, merges results.
4. **Diff.** Compute structural diff against the previous observation, including extension fragments.
...
8. **Act.** Execute via the appropriate VM service extension (host or extension).
9. **Notify.** Call `onActionExecuted()` on every extension.
...
### 11.2 Extension fragments

Per-extension structured data merged under `observation.extensions.<namespace>`. Each extension owns its key.

### 11.3 Diff

The diff covers both core and extension fragments. Each extension's diff is computed independently (the harness keeps the previous fragment and computes a structural delta). Extensions that don't define a delta-friendly shape get a "previous vs current" pair instead of a structured diff; this is the fallback for extensions with opaque blobs.

### 11.4 Budget management

Default total observation budget: 6KB serialized (~2K tokens). Distributed as:
- Core: 4KB
- Extensions: 2KB total, distributed by configured per-extension budgets
...
### 12.2 Extension tools

Anything extensions contribute. Examples from reference extensions:
...
Core tools target semantics node IDs (stable per session). Extension tools target whatever the extension defines — provider IDs, route paths, request IDs. The extension owns identifier stability for its own targets.
...
- **Per-turn diff.** Host-authored, single-turn, ~200-500 tokens typical. Now includes extension diffs.
- **Last N actions.** Last 3-5 verbatim, including outcomes.

## 14. Trajectory persistence

JSONL per session. Each record includes which extensions contributed and their fragment versions, so trajectories remain readable even if an extension is later removed or upgraded.
...
JSON-schema-constrained for `{action: {tool, args}, summary_update, rationale?, wait_strategy?}`. Schema is dynamically composed each turn from the current merged tool list (different extensions → different available tools → different valid action shape).
...
**MoE with strong agentic posture.** 35B total parameters, ~3B active per forward pass. Reported benchmarks show SWE-bench Verified 73.4, Terminal-Bench 2.0 51.5, MCPMark 37.0. The model is trained for tool-use as a first-class workload. The merged tool list of ~14 tools (host + 3 reference extensions) is well within the size class this model handles competently; tool-selection accuracy is unlikely to be the bottleneck.
...
**Native context is 262,144 tokens.** Massive. Earlier concerns about extension observation budget pressure are essentially moot at this context length. We can carry rich observation history in-context; the limit is more about model attention quality at long context than about hitting the limit.
...
- **Stability budget expired with extensions still busy.** Capture anyway, tag observation with busy extensions. Not a failure.
...
- **Extension throws during `observe()`, `busyState()`, or `onActionExecuted()`.** Logged, that turn proceeds without the extension's contribution. 3 consecutive failures auto-disable the extension.
- **Extension throws during `initialize()`.** Extension marked failed at startup; session continues without it. Logged loudly.
...
**Core:**
- `leonard_flutter` — the host binding package. `LeonardBinding`, extension contract, core tools, stability policies, VM service extensions.
...
- `leonard_router` — routing observation + `navigate` tool. Targets Flutter's built-in `Router` and `Navigator` primitives directly. No third-party routing dependency. Validates the routing-diversity solution by demonstrating the contract can express the most general case; framework-specific extensions (`go_router`, `auto_route`, `beamer`) become straightforward to author by following this pattern.
...
**Documentation:**
- AGENTS.md template
- Extension authoring guide (see §19)

Three extensions is the minimum to prove the contract works across the three extension shapes (observation, lifecycle hook, action contribution). Choosing raw `Router`/`Navigator` over `go_router` for the routing reference is deliberate: it stresses the contract more honestly (no clean RouterDelegate API to lean on, just whatever the user has wired up), avoids picking a winner among third-party routers, and produces an extension that works against any app without imposing a routing-framework choice on the host.

### Not in v1

- A Marionette compatibility extension. Nice-to-have but not required for the design to validate; revisit once the contract is stable.
- Adversarial exploration.
- Trajectory replay under `integration_test`.
- Multi-device parallel sessions.
- DevTools panel features beyond the three v1 sub-panels — long-form trajectory analytics, batch session management UI, extension authoring tooling, replay UI.
- Trajectory-based fine-tuning.
- Custom gestures beyond the discrete kinds in §12.1.
- Demonstration recording.
- Hot-reload-aware sessions.
- Auto-discovery of extensions from `pubspec.yaml`.

## 19. Extension authoring guidance (v1 documentation deliverable)

Not a section of the host, but a deliverable: a guide for writing an extension. Includes:

- The contract (§7) with worked examples.
- The reference extensions as readable source.
...
## 20. Wrapping existing tools as extensions

The extension contract is general enough to wrap existing tools, with one caveat: any existing tool that owns its own `WidgetsBinding` can't be wrapped via composition.

### 20.1 Marionette as an extension (post-v1, contingent)

Marionette is the most relevant existing tool. It's reasonable to want a `MarionetteCompatibilityExtension` that exposes Marionette's familiar tool set (`get_interactive_elements`, `tap`, `enter_text`, `scroll_to`, `take_screenshots`, `get_logs`, `hot_reload`) through the host.

Three approaches, in viability order:

**Inheritance (preferred).** `LeonardBinding extends MarionetteBinding`. Inherits all `ext.flutter.marionette.*` extensions; the extension is a Dart-side shim that exposes them through the contract. Contingent on `MarionetteBinding` being open to extension (Apache 2.0 license is permissive but the class itself may be hostile to subclassing). Verify in source before committing.

**Reimplementation.** Reimplement Marionette's primitives in our own binding. Marionette's surface is small enough that this is bounded work. Loses free updates but avoids inheritance fragility. The reimplemented extension would be `leonard_marionette_compat`, intentionally API-shaped after Marionette without sharing code.

**Coexistence.** Not viable. Two bindings cannot coexist in a single Flutter app.

The right move is to ship v1 without Marionette compatibility, validate the contract with the three reference extensions, then attempt the inheritance approach in v1.1 once the contract is stable. If inheritance proves brittle, fall back to reimplementation.

### 20.2 Generalizing the lesson

Any existing package that owns its own framework-level hooks (binding, error handler, scheduler integration) faces the same constraint. Extensions for these tools fall into one of:

- **Configuration extensions:** the existing tool exposes a config knob; the extension sets it.
- **Composition extensions:** the existing tool is a normal package without binding ownership; the extension instantiates it and exposes its API.
- **Subclass extensions:** the existing tool owns a binding; the extension requires our binding to subclass theirs.
- **Reimplementation extensions:** the existing tool is closed to subclassing; the extension reimplements relevant behavior.

This taxonomy belongs in the extension authoring guide.
...
- Should extensions be able to declare hard dependencies on other extensions (`leonard_dio` requires `leonard_logging`)? Default position: no. Extensions should be independent. Revisit if it becomes a real friction.
- For extension-contributed tools that target extension-owned identifiers (`provider_id`, `request_id`), how does the agent learn what valid IDs look like? Default: each extension's observation fragment includes the IDs it'll accept as inputs to its tools. Document this convention.
...
- **Extension contract churn.** If we change the contract after the ecosystem starts publishing extensions, we break them. Mitigation: keep v1 contract minimal, mark it explicitly experimental, and version it. Don't promise stability until v2.
- **Reference extensions not representative.** If raw `Router`/`Navigator`, Riverpod, and Dio cover 80% of cases, great. If not, the contract may need additions. Mitigation: actually try the reference extensions against real apps before declaring the contract done.
- **Extension observation budget pressure.** Three extensions at 1KB each plus core at 4KB is 7KB; this fits comfortably in the 262K-token Qwen3.6 context but tighter hardware would feel it. Mitigation: configurable budgets, plus document expected token costs prominently.
- **Extension authors disagree about identifiers.** `Router` extensions use route names or paths; Riverpod uses provider IDs; Dio uses request IDs. The model has to keep these distinct. Mitigation: enforce namespacing rigorously, document the convention, see whether the model actually struggles with this in practice (it might not given Qwen3.6's tool-use training).
...
- A user can install the host, configure 1-3 extensions, and run an exploration session on a representative app in under 15 minutes from the install command.
...
- A third party can write a working extension (any of the four taxonomies in §20.2) by reading only the extension authoring guide, with no need to read host source.
- The reference extensions individually compose with each other and with apps that use only some of `Router`/`Navigator`, Riverpod, and Dio. None of the reference extensions is required to run a session.
