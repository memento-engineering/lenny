...
**Why (A) is the right altitude.** The bead's claim is "the **agent** taps Sign In → drives Auth0 → resumes on Flutter." The `drive-dual` front door is the deterministic, model-free substrate that an agent loop sits on top of: every `invoke` goes through `MultiHostSession.act` → namespace routing → the per-host `VmServiceClient.executeAction` → `ext.leonard.<ns>.<tool>` — the **exact** wire path the autonomous loop uses. A GREEN here means the harness's perceive-merge-route-act machinery does the full Auth0 loop; only the *chooser* (scripted vs model) differs from the autonomous variant. That is the strongest claim m5 can make without importing model flakiness into a hardware tier.
...
> **RESOLVED — Auth0 web-session persistence (the sticky session DID bite; confirmed live 2026-06-24).** `ASWebAuthenticationSession` carries a sticky web session (cookies) across runs even after the app is uninstalled — a prior successful login (the spike's) left a session that made a re-run **skip the login form and land on the Auth0 authorize grant** (verified: after consent the native a11y tree showed `Authorize`/`Accept` + the tenant domain, no email/password fields). The spike's `o2_login.py` terminate/activate is the proven-but-**INCOMPATIBLE-here** workaround — it dropped the raw Appium app, which in the dual design would drop the Flutter channel. **The applied fix is `useEphemeralSession: true`** on the fixture app's `Auth0().webAuthentication(scheme:…).login(...)` (a one-line change in the TARGET app, not in lenny): each login starts with NO shared web session, so the email/password form **always** renders — making the e2e deterministic and exercising the full form (AC7/AC8). **Side effect:** an ephemeral session presents NO iOS consent sheet (no shared data to approve), so Step 2 (consent) is **ADAPTIVE** — accept it when present, tolerate its absence (§3.1 step 2 / AC6).
...
| **2** | **Accept the iOS consent sheet IF present (ADAPTIVE).** | `invoke --tool native.press --args '{"key":"consent_accept"}'` | `AppiumBackend.press('consent_accept')` → `POST /session/{id}/alert/accept`. The consent sheet is a **SpringBoard process, NOT in `/source`** — the alert endpoint is the only way. **ADAPTIVE, no hard assert:** with `useEphemeralSession` (§3.0a) there is NO consent sheet, so a miss (`ok:false` / no alert open) is a **non-fatal no-op**; when a sheet IS present it blocks the form, so accept it. The email step (next) retries until the form renders regardless. |
| **3** | **Clear+type email (RETRY-until-ok).** | `invoke --tool native.enter_text --args '{"xpath":"//XCUIElementTypeTextField[@name=\"Email address\"]","text":"<email>"}'` (text from `AUTH0_EMAIL`) | **Retry-until-ok (bounded):** the Auth0 form renders ASYNCHRONOUSLY after consent (the spike slept 7s here) — a single shot races the page load and gets "no element matched selector". `enter_text` **clears before typing** inside the backend (`POST /clear` then `POST /value`, `appium_backend.dart:467-468`). Returns `value:{via, readback, masked:false}`. **Assert** `result['value']['readback'] == <email>` and `result['value']['masked'] == false`. Steps 4–5 (password, Continue) likewise retry-until-ok (same async-rendered form). |
...
6. **Consent is handled adaptively via the native alert endpoint.** *Check (T2):* `drive-dual invoke --tool native.press --args '{"key":"consent_accept"}'` is issued; under `useEphemeralSession` there is NO consent sheet so this is a non-fatal no-op (NOT asserted `ok:true`), and the Auth0 email/password form renders for the next step. When a consent sheet IS present (non-ephemeral), accepting it via `/alert/accept` is what unblocks the form. (The proof that the alert endpoint works is AC1's `consent_accept` unit vector + the live form rendering after this step.)
