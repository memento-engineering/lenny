...
- **The backend is swappable at two levels — Appium is the FIRST impl, not a commitment.**
  ...
  2. *At this (m4) launch layer:* `--native-host <path>` spawns the host as a binary, not a package import — so an entirely different host (different backend, even non-Dart) that speaks `ext.leonard.*` + prints `LEONARD_HOST_READY` + a `ws://` URI is swappable without m4 changing.
